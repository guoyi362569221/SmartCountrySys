...
var Canvas2Image = function () {
	...
	function saveFile (strData , suffix, pnm) {
...
//		var curdtstr = dateToStr( "yyyyMMddhhmm",new Date());
//		pnm = pnm ? pnm : $("#productTitle").find('div.imgArea_productTitle').text().replace(/\s+/g , "");
		pnm = pnm == undefined || pnm == "" ? "图片" : pnm;
		a.href = strData;
		a.download = pnm + '.' + suffix;
		a.name = pnm + '.' + suffix;
		a.click();
	}
	 ...
	 */
	var saveAsImage = function (canvas, width, height, type, pnm) {
		if ($support.canvas && $support.dataURL) {
			...
			if (/bmp/.test(type)) {
				...
				saveFile(makeURI(strData, downloadMime) , t, pnm);
			} else {
				var strData = getDataURL(canvas, type, width, height);
				saveFile(strData.replace(type, downloadMime) , t, pnm);
			}
		}
	};
	
	/**
	 * getPrintImageData 获取打印数据
	 * @param canvasElement
	 * @param {String} image type
	 * @param {Number} [optional] png width
	 * @param {Number} [optional] png height 
	 ...
	 * @update by PanRS
	 */
	var getPrintImageData = function (canvas, width, height, type) {
		if ($support.canvas && $support.dataURL) {
			if (type == undefined) { type = 'png'; }
			type = fixType(type);
			var dataStr = null;
			if (/bmp/.test(type)) {
				var data = getImageData(scaleCanvas(canvas, width, height));
				var strData = genBitmapImage(data);
				dataStr = makeURI(strData, downloadMime);
			} else {
				var strData = getDataURL(canvas, type, width, height);
				dataStr = strData.replace(type, downloadMime) ;
			}
			return dataStr;
		}
	};
	...
	return {
		getPrintImageData : getPrintImageData,
