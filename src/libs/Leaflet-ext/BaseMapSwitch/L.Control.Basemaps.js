    ...
    onAdd: function (map) {
        ...
        this.options.basemaps.forEach(function (d, i) {
            ...
            if (d.options.iconURL) {
                ...
                var x = this.options.tileX;
                var y = this.options.tileY;
                var z = this.options.tileZ;

                if (d instanceof L.TileLayer._3Clear) {

                    let oo = "00000000";
                    let xx = x.toString(16);
                    x = "C" + oo.substring(0, 8 - xx.length) + xx;

                    y = y.toString(16);
                    y = 'R' + oo.substr(0, 8 - y.length) + y;
                ...
                var url = L.Util.template(d._url, L.extend({
                    ...
                    x: x,
                    y: d.options.tms ? d._globalTileRange.max.y - coords.y : y,
                    z: z
                }, d.options));
            ...
            L.DomEvent.on(basemapNode, 'click', function () {
                ...
                if (d != this.basemap) {
                    ...
                    L.DomUtil.removeClass($('#' + map.getContainer().id + ' .basemap.active')[0], 'active');
                    L.DomUtil.addClass(basemapNode, 'active');

                    var altIdx = (i + 1) % this.options.basemaps.length;

                    L.DomUtil.removeClass($('#' + map.getContainer().id + ' .basemap.alt')[0], 'alt');
                    L.DomUtil.addClass($('#' + map.getContainer().id + ' .basemap')[altIdx], 'alt');
                }
            }, this);
